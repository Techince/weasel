    ...
    if tonumber(num2) == 0 then
        ...
        elseif lens < 9 then
            result = formatNum(string.sub(num1, 1, -5), flag) ..
                digitUnit[1] .. formatNum(string.sub(num1, -4, -1), flag)
        elseif lens < 13 then
...
local function number_translatorFunc(num)
    ...
    if numberPart.dot ~= "" then
        table.insert(result,
            ...
            number2zh(numberPart.dec, 1), "〔大写〕" })
        table.insert(result,
            { number2cnChar(numberPart.int, 0, { "万", "亿" }, { "〇", "一", "十", "点" }) ..
            number2zh(numberPart.dec, 0), "〔小写〕" })
    else
        table.insert(result, { number2cnChar(numberPart.int, 1, { "萬", "億" }, { "零", "壹", "拾", "" }), "〔大写〕" })
        table.insert(result, { number2cnChar(numberPart.int, 0, { "万", "亿" }, { "〇", "一", "十", "" }), "〔小写〕" })
    end
    -- table.insert(result,
    --     {
