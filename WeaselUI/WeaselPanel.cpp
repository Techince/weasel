...
inline void LoadIconNecessary(T0& a, T1& b, T2& c, int d)
{
	...
	if (b.empty())
	{
		c.LoadIcon(d, STATUS_ICON_SIZE, STATUS_ICON_SIZE, LR_DEFAULTCOLOR);
	}
	else
	{
		c = (HICON)LoadImage(nullptr, b.c_str(), IMAGE_ICON, STATUS_ICON_SIZE, STATUS_ICON_SIZE, LR_LOADFROMFILE);
	}
}
...
void WeaselPanel::Refresh()
{
	...
	bool show_schema_menu = std::regex_search(m_ctx.preedit.str, std::wsmatch(), std::wregex(L"^〔方案选单〕", std::wregex::icase));
...
bool WeaselPanel::_DrawCandidates(CDCHandle& dc, bool back)
{
	...
	if (back) {
		...
		if (COLORNOTTRANSPARENT(m_style.candidate_back_color)
			|| COLORNOTTRANSPARENT(m_style.candidate_border_color))	// if transparent not to draw
		{
			for (size_t i = 0; i < m_candidateCount && i < MAX_CANDIDATES_COUNT; ++i) {
...
void WeaselPanel::DoPaint(CDCHandle dc)
{
	...
	if (!hide_candidates) {
		bool over_bottom{};
		auto auxrc = m_layout->GetAuxiliaryRect();
		auto preeditrc = m_layout->GetPreeditRect();
		if (m_style.vertical_auto_reverse && m_style.layout_type == UIStyle::LAYOUT_VERTICAL)
		{
			RECT rcWorkArea;
			memset(&rcWorkArea, 0, sizeof(rcWorkArea));
			auto hMonitor = MonitorFromRect(m_inputPos, MONITOR_DEFAULTTONEAREST);
			if (hMonitor)
			{
				MONITORINFO info;
